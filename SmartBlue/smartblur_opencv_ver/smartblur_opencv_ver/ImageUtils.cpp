...
Mat ImageUtils::buildKernelImage(const FocusBlur* focusBlur) {
	...
	Mat kernelImage(size, size, CV_8UC3, Scalar(0,0,0));
	...
	//kernelPainter.end();
	circle(kernelImage, Point(0.5 + kernelImage.cols / 2.0, 0.5 + kernelImage.rows / 2.0), radius, CV_RGB(255, 255, 255),-1);
	...
	for (int y = 0; y<size; y++) {
		for (int x = 0; x<size; x++) {
			...
			if (dist <= radius) {
				...
				int curValue = kernelImage.at<Vec3b>(x, y)[2];
				...
				//kernelImage.at<Vec3b>(x, y) = Vec3b(curValue, curValue, curValue);
				kernelImage.at<Vec3b>(x, y)[0] = curValue;
				kernelImage.at<Vec3b>(x, y)[1] = curValue;
				kernelImage.at<Vec3b>(x, y)[2] = curValue;

			}
		}
	}
	namedWindow("kernelÍ¼Ïñ");
	imshow("kernelÍ¼Ïñ", kernelImage);
	return kernelImage;
}
...
void ImageUtils::fillMatrixFromImage(ProcessingContext *processingContext, const CurrentChannel channel) {
	for (int y = 0; y<processingContext->height; y++) {
		...
		Vec3b *line = processingContext->inputImage.ptr<Vec3b>(y);
		for (int x = 0; x<processingContext->width; x++) {
			...
			case RED:   value = line[x][2]; break;
			case GREEN: value = line[x][1]; break;
			case BLUE:  value = line[x][0]; break;
			case GRAY:  value = line[x][2]; break;
			}
...
void ImageUtils::fillImageFromMatrix(ProcessingContext* processingContext, const CurrentChannel channel) {
	...
	for (int y = 0; y < processingContext->height; y++) {
		...
		Vec3b *line = processingContext->outputImage.ptr<Vec3b>(y);
		for (int x = 0; x < processingContext->width; x++) {
			...
			case RED:   line[x][0] = 0;line[x][1] = 0;line[x][2] = value; break;
			case GREEN: line[x][0] = 0;line[x][1] = value;line[x][2] = 0; break;
			case BLUE:  line[x][0] = value;line[x][1] = 0;line[x][2] = 0; break;
			case GRAY:  line[x][0] = value;line[x][1] = value;line[x][2] = value; break;
			}
		}
	}
}
