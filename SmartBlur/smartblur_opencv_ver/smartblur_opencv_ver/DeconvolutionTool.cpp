...
void DeconvolutionTool::initFFT(const InputArray inputImage) {
	removeFFTObjects();
	/*QTime time;
	time.start();
	...
	// Read image size
	width = inputImage.getMat().cols;
	height = inputImage.getMat().rows;
...
bool DeconvolutionTool::doDeconvolution(InputArray inputImage, OutputArray outputImage, Blur* blur) {
	//isProcessingCancelled = false;
	// Create kernel
	...
	processingContext->inputImage = inputImage.getMat();
	processingContext->outputImage = outputImage.getMat();
	...
	if (blur->mode == PREVIEW_GRAY) {
		...
		String progressText = "High-Quality";
	...
	namedWindow("Êä³ö",1);
	imshow("Êä³ö", processingContext->outputImage);
	...
	for (int y = 0; y<HEIGHT; y++) {
		for (int x = 0; x<WIDTH; x++) {
			...
			if (abs(x - WIDTH / 2)<(size - 2) / 2 && abs(y - HEIGHT / 2)<(size - 2) / 2) {
				...
				value = kernelImage.at<Vec3b>(xLocal, yLocal)[2];
			}
...
void DeconvolutionTool::visualizeFFT(fftw_complex* fft, const int WIDTH, const int HEIGHT, String path) {
	Mat visualImage(WIDTH, HEIGHT, CV_8UC3);
	...
	for (int y = 0; y<HEIGHT; y++) {
		for (int x = 0; x<WIDTH; x++) {
			...
			visualImage.at<Vec3b>(x, y)[0] = curAmpl;
			visualImage.at<Vec3b>(x, y)[1] = curAmpl;
			visualImage.at<Vec3b>(x, y)[2] = curAmpl;
		}
	}

	//visualImage.save(path);
	//imwrite(path, visualImage);
}
